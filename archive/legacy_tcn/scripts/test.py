...
# Use the same normalization used during feature extraction
from extract_pose import _normalize

# ====== PATHLAR ======
PROJECT_ROOT = r"C:\Users\ali\Projects\fall_research"
TCN_MODEL_PATH = os.path.join(PROJECT_ROOT, "experiments", "FD-02", "best.pt")
    ...
    try:
        while True:
            for cam in cameras:
                ...
                # Normalize keypoints the same way as during training (root+scale)
                try:
                    nkp = _normalize(keypoints, kp_conf)  # (17,2) normalized
                except Exception:
